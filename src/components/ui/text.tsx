...
const typographyVariant = cva("", {
  variants: {
    variant: {
      ...
      anchor:
        "text-[0.875rem] underline underline-offset-4 decoration-neutral-500 hover:decoration-neutral-700",
    },
    weight: {
      light: "font-light",
      regular: "font-normal",
...
interface TextProps
  extends VariantProps<typeof typographyVariant>,
  ...
  if (href) {
    return (
      <Link
        ...
        {...props}
        className={cn(
          typographyVariant({ gradient, variant, weight, lightness }),
          className,
        )}
      ></Link>
    );
